...
public class Earth extends Renderable {
   ...
   private double[] getNearFarDistances(final Scene scene) {
      ...
      final Tuple3d[] screenCorners = new Tuple3d[] { new Tuple3d(0, 0, 1.0), new Tuple3d(w, 0, 1.0), new Tuple3d(w, h, 1.0), new Tuple3d(0, h, 1.0) };
      ...
      for (final Tuple3d screenCorner : screenCorners) {
         ...
         if (worldXyz != null) {
            ...
            if (Double.isFinite(distance)) {
               near = Math.max(1.0, Math.min(near, distance - Earth.MAX_DEPTH_RATIO));
               far = Math.max(far, distance + Earth.MAX_DEPTH_RATIO);
            }
         }
      }

      if (near > (far / Earth.MAX_DEPTH_RATIO)) {
         near = far / Earth.MAX_DEPTH_RATIO;
      }
